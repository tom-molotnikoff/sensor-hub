...
export interface components {
    schemas: {
        ...
        AggregatedReadingsResponse: {
             ...
             * @enum {string}
             */
            aggregation_function: "none" | "avg" | "count" | "last" | "min" | "max" | "increase";
    ...
    getReadingsBetweenDates: {
        parameters: {
            query: {
                ...
                /**
                 * @description Override the aggregation function. Defaults are looked up per-measurement-type (e.g. `avg` for temperature, `count` for binary sensors). `increase` reports how much a running counter, such as an energy meter, rose within each bucket, treating a drop as a counter reset. Only meaningful when aggregation is not `raw`. Requires `type`; the request is rejected with 400 without it.
                 * @example avg
                 */
                aggregation_function?: "avg" | "count" | "last" | "min" | "max" | "increase";
            };
            header?: never;
            path?: never;
            cookie?: never;
